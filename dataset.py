...
class PriceDataset(Dataset):
    def __init__(self, 
                 ...
                 end_date, 
                 is_training=True, 
                 in_columns=['USD_Price','BrentOil_Price', 'Copper_Price', 'CrudeOil_Price', 'Gasoline_Price', 'Gold_Price', 'NaturalGas_Price', 'Platinum_Price', 'Silver_Price', 'AUD_Price', 'CNY_Price', 'EUR_Price', 'GBP_Price', 'HKD_Price', 'JPY_Price'], 
...
#   column_mean = df[column].mean()
    ...
    # 이상치를 평균값으로 대체
#    df[column] = df[column].mask((df[column] < lower_bound) | (df[column] > upper_bound), column_mean)
#    return df
...
def make_features(start_date, end_date, in_columns, out_columns, input_days, 
    ...
    #table.drop(['CrudeOil_Price', 'HKD_Price', 'Silver_Price','AUD_Price','Copper_Price'], axis=1)
    ...
    #이상치 바꾼거#
    #df = replace_outliers_with_mean(df, 'USD_Price')
    ...
    for symbol in SYMBOLS:
        ...
        if price_column in df.columns:
            # MACD 및 Signal Line 계산 및 추가
            #df[f'{symbol}_MACD'], df[f'{symbol}_Signal_Line'] = calculate_macd(df[price_column], 12, 26, 9)

            # RSI 계산 및 추가
            #df[f'{symbol}_RSI'] = calculate_rsi(df[price_column], 14)

            # 변동성 계산 및 추가
            #df[f'{symbol}_Volatility'] = calculate_volatility(df[price_column], 20)

            # 결측치 처리
            df.fillna(0, inplace=True)

            # 입력 컬럼 목록에 추가
            #in_columns.extend([f'{symbol}_MACD', f'{symbol}_Signal_Line', f'{symbol}_RSI', f'{symbol}_Volatility'])
    ...
    # 선형 보간법을 사용하여 결측치 처리
    #df['USD_Price_MA5'] = df['USD_Price_MA5'].interpolate(method='linear')
    #df['USD_Price_MA5'] = df['USD_Price_MA5'].interpolate(method='linear')
    #in_columns.append('USD_Price_MA5')
    #df=table.drop(['CrudeOil_Price', 'HKD_Price', 'Silver_Price','AUD_Price','Copper_Price'], axis=1)
    ...
    # 결측치가 있는 행의 비율을 확인합니다.
    #missing_percentage = df.isnull().mean() * 100
    #print(missing_percentage)

    # 전체 데이터에서 결측치가 있는 컬럼의 수를 확인합니다.
